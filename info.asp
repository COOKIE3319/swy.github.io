...
rs.Close
Set rs = Nothing
If Not inull(i_slideshow) Then
	Set matches = str_execute("<img[^>]*src\s*=\s*['"&CHR(34)&"]?([\w/\-\:.]*)['"&CHR(34)&"]?[^>]*>",i_slideshow)
	i_slideshow = ""
	For Each Match in matches
		'i_slideshow = i_slideshow & Match.Value'获取整个img
		i_slideshow = i_slideshow&spacer&Match.SubMatches(0)'只取src
		spacer = "|"
	Next
End If
If i_member = 1 Then
...
End If
i_title = iif((inull(i_seoname)),i_name&" - "&site_name,i_name&" - "&i_seoname&" - "&site_name)
...
End If
Function i_prev()
	Set rs = ado_query("select id,i_enable,i_order,i_name,i_parent,i_ifrec,i_ifhot,i_iftop,i_ifcov,i_bold,i_italic,i_color,i_picture,i_target,i_scontent,i_hits,i_date from cms_info where i_parent in ("&c_sub&") and i_order > "&i_order&" order by i_order asc")
	If rs.EOF Then
		i_prev = "暂无"
	Else
...
Function i_next()
	Set rs = ado_query("select id,i_enable,i_order,i_name,i_parent,i_ifrec,i_ifhot,i_iftop,i_ifcov,i_bold,i_italic,i_color,i_picture,i_target,i_scontent,i_hits,i_date from cms_info where i_parent in ("&c_sub&") and i_order < "&i_order&" order by i_order desc")
	If rs.EOF Then
		i_next = "暂无"
	Else
