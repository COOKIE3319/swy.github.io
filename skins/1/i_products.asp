			...
			<div class="l20">
					...
					<div id="info_slideshow">
						...
						<div class="hd">
							<ul class="l4">
								...
								arr_slideshow = Split(i_slideshow, "|")
								For i = 0 To UBound(arr_slideshow)
									echo "<li class=""xx15 mb4""><div class=""img""><div class=""slideshow_thumbnail""><img src="""&arr_slideshow(i)&"""/></div></div></li>"
								Next
								%>
							</ul>
						</div>
				...
				<div class="x5">
					...
					<div id="info_parmeter">
						<ul>
							<%If Not inull(i_a) Then%><li><%=site_p1%>：<%=i_a%></li><%End If%>
							<%If Not inull(i_b) Then%><li><%=site_p2%>：<%=i_b%></li><%End If%>
							<%If Not inull(i_c) Then%><li><%=site_p3%>：<%=i_c%></li><%End If%>
							<%If Not inull(i_d) Then%><li><%=site_p4%>：<%=i_d%></li><%End If%>
							<%If Not inull(i_e) Then%><li><%=site_p5%>：<%=i_e%></li><%End If%>
						</ul>
					</div>
					<div class="l10">
						<%If site_order <> 2 Then%><div class="x4"><a class="btn bg-red btn-block" href="<%=site_dir%>order.asp?id=<%=i_id%>"><span class="icon-shopping-cart"> 我要订购</span></a></div><%End If%>
						<%If site_guestbook <> 3 Then%><div class="x4"><a class="btn bg-red btn-block" href="<%=site_dir%>guestbook.asp"><span class="icon-users"> 在线留言</span></a></div><%End If%>
						<%If site_comment <> 3 Then%><div class="x4"><a class="btn bg-red btn-block" href="<%=site_dir%>comment.asp?id=<%=i_id%>"><span class="icon-comments"> 我要评论</span></a></div><%End If%>
					</div>
				</div>
			</div>
			<%If i_attach <> "" then%>
