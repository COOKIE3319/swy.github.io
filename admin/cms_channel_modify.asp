...
If Not inull(rf("save")) Then
	Set rs = ado_mquery("select * from cms_channel where id = "&rqs("id")&"")
	...
	rs("c_rh") = rf("c_rh")
	rs("c_twidth") = rf("c_twidth")
	rs("c_theight") = rf("c_theight")
	rs("c_slideshow") = rf("c_slideshow")
	rs("c_mdate") = Now()
	rs.Update
	rs.Close
	Set rs = Nothing
...
KindEditor.ready(function(K) {
	...
	K.create('#c_mcontent');
	K.create('#c_slideshow',{
		items:[
			'image', 'multiimage', 'undo', 'redo', 'fullscreen'
		],
		cssData:'img { width: 100px !important; height: 100px !important; margin: 5px; border: 1px solid #EEE; padding: 4px;}'
	});
	var editor = K.editor();
				...
				<div class="l10">
					...
					<div class="x12 dn">
						<div class="form-group">
							...
							<div class="field">
								<textarea id="c_mcontent" class="input" name="c_mcontent" row="5" /><%=str_editor(rs("c_mcontent"))%></textarea>
								<div class="input-note">留空后自动识别电脑详细内容</div>
							</div>
						</div>
					</div>
					<div class="x12 dn">
						<div class="form-group">
							...
							<div class="field">
								<textarea id="c_mscontent" class="input" name="c_mscontent" row="5" /><%=str_editor(rs("c_mscontent"))%></textarea>
								<div class="input-note">留空后自动识别电脑简短内容</div>
							</div>
						</div>
					</div>
					<div class="x12">
						<div class="form-group">
							<div class="label"><label for="c_slideshow">多图</label></div>
							<div class="field">
								<textarea id="c_slideshow" class="input" name="c_slideshow" row="5" /><%=str_editor(rs("c_slideshow"))%></textarea>
							</div>
						</div>
					</div>
					<div class="x12">
					...
					<div class="x2">
						<div class="form-group">
							<div class="label"><label for="c_twidth">缩略图宽高比</label></div>
							<div class="field">
								<div class="l10">
									<div class="x6"><input id="c_twidth" class="input" name="c_twidth" type="text" maxlength="255" size="60" value="<%=rs("c_twidth")%>" data-validate="required:必须填写,plusinteger:必须为正整数" /></div>
									<div class="x6"><input class="input" name="c_theight" type="text" maxlength="255" size="60" value="<%=rs("c_theight")%>" data-validate="required:必须填写,plusinteger:必须为正整数"/></div>
								</div>
							</div>
						</div>
					</div>
