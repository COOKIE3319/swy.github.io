...
If Not inull(rf("save")) Then
	Set rs = ado_query("select * from cms_admin where a_name = '"&a_name&"' and id <> "&rqs("id")&" ")
	If Not rs.EOF Then
	...
	Set rs = ado_mquery("select * from cms_admin where id = "&rqs("id")&"")
	rs("a_enable") = iif((rqs("id") = 1),1,rf("a_enable"))
	rs("a_name") = rf("a_name")
