...
If Not inull(rf("save")) Then
	...
	rs("site_mcopyright") = rf("site_mcopyright")
	rs("site_qrcode") = rf("site_qrcode")
	rs("site_s1") = rf("site_s1")
	...
	rs("site_thumbnail") = rf("site_thumbnail")
	rs("site_twidth") = rf("site_twidth")
	rs("site_theight") = rf("site_theight")
	rs("site_weixin") = rf("site_weixin")
	rs("site_wxqrcode") = rf("site_wxqrcode")
	rs("site_weibo") = rf("site_weibo")
	rs("site_email") = rf("site_email")
	rs("site_address") = rf("site_address")
	rs("site_hotkey") = rf("site_hotkey")
	rs.Update
	rs.Close
	Set rs = Nothing
	conn.Execute("update cms_skin set s_logo = '"&rf("site_logo")&"' where s_path = '"&site_skin&"'")
	conn.Execute("update cms_skin set s_logo = '"&rf("site_mlogo")&"' where s_path = '"&site_mskin&"'")
...
KindEditor.ready(function(K) {
	...
	K('#wxqrcode').click(function() {
		editor.loadPlugin('image', function() {
			editor.plugin.imageDialog({
			imageUrl : K('#site_wxqrcode').val(),
			clickFn : function(url, title, width, height, border, align) {
				K('#site_wxqrcode').val(url);
				editor.hideDialog();
				}
			});
		});
			...
			<form method="post" class="l10">
				<div class="form-group x3">
					...
					<div class="field">
						...
						<label class="btn"><input name="site_mode" type="radio" value="2" <%=iif(rs("site_mode") = 2," checked=""checked""","")%>/>伪静态</label>
						<div class="input-note">伪静态要求IIS7.0以上并安装url重写组件</div>
					</div>
				</div>
				<div class="form-group x3">
				...
				<div class="form-group xx25 dn">
					<div class="label"><label for="site_murl">手机网址</label></div>
					<div class="field">
						<input id="site_murl" class="input" name="site_murl" type="text" size="60" value="<%=rs("site_murl")%>" data-validate="url:请填写合法的URL地址" />
						<div class="input-note">示例：http://m.xxxx.com</div>
					</div>
				</div>
				<div class="form-group x1 dn">
					<div class="label"><label for="site_mclose">手机站</label></div>
					<div class="field">
						<select id="site_mclose" class="input" name="site_mclose">
							<option value="0" <%=iif(rs("site_mclose") = 0,"selected=""site_mclose""","")%>>打开</option>
							<option value="1" <%=iif(rs("site_mclose") = 1,"selected=""site_mclose""","")%>>关闭</option>
						</select>
					</div>
				</div>
				<div class="fc"></div>
				...
				<div class="form-group x7">
					...
					<div class="field">
						...
						<div class="input-note">为了让搜索引擎更好的收录您的网站，请仔细填写关键词，多个关键词中间用英文逗号","分割</div>
					</div>
				</div>
				<div class="fc"></div>
				...
				<div class="form-group x12">
					<div class="label"><label for="site_hotkey">热门关键词</label></div>
					<div class="field">
						<input id="site_hotkey" class="input" name="site_hotkey" type="text" size="60" value="<%=rs("site_hotkey")%>" />
						<div class="input-note">热门关键词之间用“|”分割</div>
					</div>
				</div>
				<div class="form-group x3">
				...
				<div class="form-group x3 dn">
					<div class="label"><label for="site_mlogo">手机端Logo</label> <span class="badge bg-dot cp" id="mlogo">上传</span></div>
					<div class="field">
						<input id="site_mlogo" class="input" name="site_mlogo" type="text" size="60" value="<%=site_mlogo%>" />
					</div>
				</div>
				<div class="form-group x3">
					<div class="label"><label for="site_qrcode">二维码</label> <span class="badge bg-dot cp" id="qrcode">上传</span></div>
					<div class="field">
						<input id="site_qrcode" class="input" name="site_qrcode" type="text" size="60" value="<%=rs("site_qrcode")%>" />
					</div>
				</div>
				<div class="form-group x3">
					<div class="label"><label for="site_wxqrcode">微信二维码</label> <span class="badge bg-dot cp" id="wxqrcode">上传</span></div>
					<div class="field">
						<input id="site_wxqrcode" class="input" name="site_wxqrcode" type="text" size="60" value="<%=rs("site_wxqrcode")%>" />
					</div>
				</div>
				<div class="fc"></div>
				...
				<div class="form-group x2">
					<div class="label"><label for="site_weibo">微博</label></div>
					<div class="field">
						<input id="site_weibo" class="input" name="site_weibo" type="text" maxlength="255" size="60" value="<%=rs("site_weibo")%>" />
					</div>
				</div>
				<div class="form-group x2">
					<div class="label"><label for="site_email">邮件</label></div>
					<div class="field">
						<input id="site_email" class="input" name="site_email" type="text" maxlength="255" size="60" value="<%=rs("site_email")%>" />
					</div>
				</div>
				<div class="form-group x2">
					<div class="label"><label for="site_address">地址</label></div>
					<div class="field">
						<input id="site_address" class="input" name="site_address" type="text" maxlength="255" size="60" value="<%=rs("site_address")%>" />
					</div>
				</div>
				<div class="form-group x2">
					<div class="label"><label for="site_weixin">微信号</label></div>
					<div class="field">
						<input id="site_weixin" class="input" name="site_weixin" type="text" maxlength="255" size="60" value="<%=rs("site_weixin")%>" />
					</div>
				</div>
				<div class="fc"></div>
				<div class="form-group xx13">
					<div class="label"><label for="site_thumbnail">缩略图等比</label></div>
					<div class="field">
						<select id="site_thumbnail" class="input" name="site_thumbnail">
							<option value="0" <%=iif(rs("site_thumbnail") = 0,"selected=""selected""","")%>>否</option>
							<option value="1" <%=iif(rs("site_thumbnail") = 1,"selected=""selected""","")%>>是</option>
						</select>
					</div>
				</div>
				<div class="form-group xx26">
					<div class="label"><label for="site_twidth">缩略图宽高比</label></div>
					<div class="field">
						<div class="l10">
							<div class="x6"><input id="site_twidth" class="input" name="site_twidth" type="text" size="60" value="<%=rs("site_twidth")%>" data-validate="required:必须填写, plusinteger:必须为正整数" /></div>
							<div class="x6"><input id="site_theight" class="input" name="site_theight" type="text" size="60" value="<%=rs("site_theight")%>" data-validate="required:必须填写, plusinteger:必须为正整数"/></div>
						</div>
					</div>
				</div>
				<div class="form-group xx16">
					...
					<div class="field">
						<input id="site_banner" class="input" name="site_banner" type="text" size="60" value="<%=rs("site_banner")%>" data-validate="required:必须填写, plusinteger:必须为正整数"/>
					</div>
				</div>
				<div class="form-group xx13">
				...
				<div class="form-group x12">
					...
					<div class="field">
						<label class="btn btn-small dn">手机版 <input name="site_s1" type="checkbox" value="1" <%=iif(rs("site_s1") = 1,"checked=""checked""","")%>/></label>
				...
				<div class="form-group x12 dn">
					<div class="label"><label for="site_mcopyright">手机端版权信息</label></div>
					<div class="field">
						<textarea id="site_mcopyright" class="input" name="site_mcopyright" row="5" /><%=str_editor(rs("site_mcopyright"))%></textarea>
					</div>
				</div>
				<div class="form-group x12">
