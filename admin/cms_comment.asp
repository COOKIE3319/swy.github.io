...
<div id="content">
	...
	<div id="right">
		...
		<div class="bd">
			<%
			If inull(rqs("id")) Then
				echo "<div class=""quote mb10"">点击相应评论内容可以显示同一信息下的所有评论</div>"
			Else
				Set rs = ado_query("select * from cms_info where id = "&rqs("id")&" ")
				If Not rs.EOF Then
					echo "<div class=""quote mb10""><a href="""&i_url(rs("id"))&""" target=""_blank"" title="""&rs("i_name")&""">"&rs("i_name")&"</a></div>"
				End If
				rs.Close
				Set rs = Nothing
			End If
			...
			<form method="post" class="form-auto">
				<table class="table table-bordered">
					...
					Else
						sql = "select * from cms_comment where c_parent = "&rqs("id")&" order by c_date desc"
					End If
					page_size = 50
					...
					<tr class="ac">
						...
						<td class="al"><a href="cms_comment.asp?id=<%=rs("c_parent")%>"><%=rs("c_content")%></a></td>
